...
`scripts/payload-executables.txt` declares that set, and the `validate` workflow fails if an undeclared executable appears or if this file stops naming one. The workflow runs on every pull request, every push to `main`, and daily on a schedule, so drift in the claim above surfaces as a failed check on the commit that caused it. `main` carries no branch protection, so the check reports drift rather than blocking it: read the check before you trust the claim.
