...
| Surplus cannot list that host yet | Pick a featured provider, or Other with an OpenAI-compatible URL Surplus already accepts. |
...
| paused | Daily cap ≈ 0 | Resume from the dashboard if the user wants traffic |
...
| Inbound USDC that is not earnings | Other inbound is balance. Earnings are Surplus relayer transfers only. |
